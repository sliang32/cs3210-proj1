...
static unsigned long long ADDER	      = 0;
...
#pragma omp for
      for (i=0; i<120; i++) {
	testref[i] = (double)lfprng();
	sum += abs(testref[i] - baseref[i]);
	printf(" lfprng: %f, base: %f.", testref[i], baseref[i]);
      }
    }    
